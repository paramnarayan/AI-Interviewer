...
`metrics.py` instruments the backend using `prometheus-client`. Current metrics:

| Metric | Type | Description |
|---|---|---|
| `stt_latency_seconds` | Histogram | Time from audio-end signal to completed transcript; buckets at 0.1 s, 0.25 s, 0.5 s, 0.75 s, 1.0 s, 1.5 s, 2.0 s, and 5.0 s |

Additional metrics (LLM latency, TTS latency, active sessions) can be added by importing `Histogram`, `Gauge`, and `Counter` from `metrics.py`.
...
- **Prometheus Observability**: `metrics.py` provides an `stt_latency_seconds` histogram for tracking STT pipeline performance. The module is structured for easy extension with additional counters and gauges.
